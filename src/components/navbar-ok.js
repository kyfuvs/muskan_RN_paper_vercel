        ...
        <View style={[
          ...
          <View style={[
            ...
            {/* <TouchableOpacity 
              onPress={() => {
                router.push('/');
                if (screenWidth < 400) setMenuVisible(false);
              }}
            >
              <Text style={[styles.link, {color: theme.colors.text || '#ffcc00'}]}>
                Home
              </Text>
            </TouchableOpacity>  */}
            {/* <Hightlight_link link_name={'Home'} link_page={'/'}/>
            <Hightlight_link link_name={'Guides'} link_page={'/guideFolder'}/> */}
            <Active_link link_name={'Home'} link_page={'/'}/>
            <Active_link link_name={'Guides'} link_page={'/guideFolder'}/>
            {/* <TouchableOpacity 
              onPress={() => {
                router.push('/guideFolder');
                if (screenWidth < 400) setMenuVisible(false);
              }}
            >
              <Text style={[styles.link, {color: theme.colors.text || '#ffcc00'}]}>
            ...
            <TouchableOpacity 
              onPress={() => {
                router.push('/compFolder');
                if (screenWidth < 400) setMenuVisible(false);
              }}
            >
              <Text style={[styles.link, {color: theme.colors.text || '#ffcc00'}]}>Components</Text>
            </TouchableOpacity>
            <TouchableOpacity 
              onPress={() => {
                router.push('/showcaseFolder');
                if (screenWidth < 400) setMenuVisible(false);
              }}
            >
              <Text style={[styles.link, {color: theme.colors.text }]}>Showcase</Text>
            </TouchableOpacity>
          </View>
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  mobileLinksContainer: {
    position: 'absolute',
    top: 56, // Position just below the navbar
    left: 0,
    right: 0,
    backgroundColor: 'rgba(63, 62, 62, 0.9)', // Semi-transparent background
    paddingVertical: 15,
    paddingHorizontal: 20,
    zIndex: 999,
  },
